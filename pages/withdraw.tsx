    ...
    return (
        <Box p={4}>
            ...
            <Flex justifyContent="center" alignItems="center" direction="column">
            {
                balance ? (
                    <>
                    <Text>Available funds to withdraw is: {balance} MATIC</Text>
                    <Button variant="outline" onClick={withdrawFunds} m={2} p={4}>Withdraw Funds</Button>
                    </>
                ) : (
                    <>
                    {wallet.address ? (<>Loading...</>) : (<>Wallet not connectd yet :(</>)}
                    </>
                )
            }
            </Flex>

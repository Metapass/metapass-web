...
function CreateForm({ setTitle, setDescription, isPaid, setFee, uploadToFirebase, setIsPaid, setSeats, setFile }: any) {
    return ( 
    <>
        <Heading m={2}>Create an event</Heading> 
        <Input m = { 2 } onChange = {
            (e) => setTitle(e.target.value) } placeholder = { "Title" }
        /> 
        ...
        /> 
        <InputGroup m = { 2 } >
        <Input type="file" p={1} accept="image/*" onChange={e => setFile(e.target.files[0])} /> 
        <InputRightAddon > <Link href="#">Ticket image for your event to be minted as NFT. Click here for sample.</Link> </InputRightAddon> 
        </InputGroup> 
        <CheckboxGroup>
        <Checkbox m={2} defaultIsChecked={true} onChange={e => setIsPaid(!isPaid)}> Paid Ticket</Checkbox> </CheckboxGroup> {
            isPaid ? (
                <InputGroup m={2}>
